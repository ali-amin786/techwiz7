...
$nav_inner = function () use ($saved_count) {
    ...
    <nav class="sidebar-nav">
        ...
        <?php if (is_logged_in()): ?>
            <a class="nav-link<?= active_nav('saved-items.php') ?>" href="<?= e(url('saved-items.php')) ?>">Saved Content (<?= (int) $saved_count ?>)</a>
        ...
        <?php else: ?>
            <a class="nav-link nav-muted js-guest-guard" href="<?= e(url('login.php')) ?>" data-guard="saved">Saved Content (Login Required)</a>
            <a class="nav-link nav-muted js-guest-guard" href="<?= e(url('login.php')) ?>" data-guard="submit">Submit Post</a>
        <?php endif; ?>
    </nav>
